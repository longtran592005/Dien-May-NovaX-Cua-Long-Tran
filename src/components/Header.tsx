...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background border-b border-border shadow-soft">
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-2xl mx-8">
          <div className="relative w-full group">
            {searchFocused && <div className="absolute inset-0 rounded-full ring-2 ring-primary -z-10"></div>}
            <input
              type="text"
              value={searchQuery}
              ...
              className="w-full pl-6 pr-14 py-3.5 rounded-full text-foreground bg-background border border-border focus:border-primary outline-none text-sm font-medium transition-colors"
            />
            ...
            {searchFocused && searchQuery.length > 0 && (
              <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-card rounded-2xl border border-border overflow-hidden">
                {suggestions.length > 0 ? (
                  <div className="py-2">
                    ...
                    <Link to={`/products?q=${encodeURIComponent(searchQuery)}`} className="block text-center py-3 text-sm text-primary font-bold hover:bg-secondary border-t border-border">
                      Xem tất cả kết quả →
                    </Link>
                  </div>
                ) : (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">💡 Gợi ý sẽ hiển thị nếu tìm thấy sản phẩm</p>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <div className="hidden md:flex items-center gap-2 ml-2 pl-2 border-l border-border">
            {isAuthenticated ? (
              ...
              <Link to="/login" className="flex items-center gap-2 gradient-primary text-white px-5 py-2 rounded-full text-sm font-bold shadow-md hover:shadow-lg hover:brightness-105 transition-all">
      ...
      <nav className="hidden md:block border-t border-border" ref={megaMenuRef}>
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-2">
            <button
              onClick={() => setMegaMenuOpen(!megaMenuOpen)}
              className={`flex items-center gap-2 px-5 py-3 font-bold text-sm transition-colors rounded-t-xl ${megaMenuOpen ? 'bg-primary text-primary-foreground' : 'hover:text-primary hover:bg-secondary'}`}
            >
            ...
            {categories.slice(0, 6).map(cat => (
              <Link
                ...
                className="px-4 py-3 text-sm font-semibold text-foreground hover:text-primary hover:bg-secondary transition-colors whitespace-nowrap rounded-lg"
              >
                {cat.name}
              </Link>
            ))}
            <div className="flex-1"></div>
            <Link to="/blog" className="px-4 py-3 text-sm font-semibold text-foreground hover:text-primary transition-colors whitespace-nowrap">
              Tin tức
            </Link>
            <Link to="/order-tracking" className="px-4 py-3 text-sm font-semibold text-foreground hover:text-primary transition-colors whitespace-nowrap">
              Tra cứu đơn
            </Link>
          </div>
          ...
          {megaMenuOpen && (
            <div className="absolute left-0 right-0 bg-card border-b border-border shadow-float origin-top">
      ...
      {/* Mobile menu (Glass) */}
      {menuOpen && (
        <div className="md:hidden bg-card border-b border-border absolute w-full top-full">
          <form onSubmit={handleSearch} className="px-4 py-4">
            <div className="relative">
              <input
                ...
                className="w-full pl-4 pr-12 py-3 rounded-xl border border-border bg-background text-sm font-medium focus:ring-2 focus:ring-primary outline-none"
              />
          ...
          <div className="px-4 pb-6 space-y-2 max-h-[70vh] overflow-y-auto">
            ...
            <Link to="/blog" className="block py-2.5 px-4 text-sm font-bold text-foreground hover:bg-secondary rounded-xl" onClick={() => setMenuOpen(false)}>
              Tin tức
            </Link>
            <Link to="/order-tracking" className="block py-2.5 px-4 text-sm font-bold text-foreground hover:bg-secondary rounded-xl" onClick={() => setMenuOpen(false)}>
              Tra cứu đơn hàng
            </Link>
            <Link to="/stores" className="block py-2.5 px-4 text-sm font-bold text-foreground hover:bg-secondary rounded-xl" onClick={() => setMenuOpen(false)}>
              Hệ thống cửa hàng
            </Link>
